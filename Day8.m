...
for i = 1:size(keystr,1)
    ...
    output = output+DeriveDigit(out(i,:), onechar, fourchar);
end

fprintf('Sum of all output values is %i.\n',output)

function output = DeriveDigit(key, onechar, fourchar)
    output = 0;
    keylength = length(key);
    for i = 1:keylength
        input = char(key(i));
        charlength = length(input);
        switch charlength
            case 2
                output = output+1*10^(keylength-i);
                continue;
            case 3
                output = output+7*10^(keylength-i);
                continue;
            case 4
                output = output+4*10^(keylength-i);
                continue;
            case 7
                output = output+8*10^(keylength-i);
                continue;
        end
        
        onecommon = findintersectnum(input,onechar);
        fourcommon = findintersectnum(input,fourchar);
        
        if charlength == 5
            if onecommon == 1
                if fourcommon == 2
                    output = output+2*10^(keylength-i);
                else
                    output = output+5*10^(keylength-i);
                end
            else
                output = output+3*10^(keylength-i);
            end
        else
            if onecommon == 2
                if fourcommon == 3
                    ...
                    output = output+9*10^(keylength-i);
                end
            else
                output = output+6*10^(keylength-i);
            end
        end
    end
end

function numchar = findintersectnum(input1, input2)
    numchar = 0;
    for i = 1:length(input2)
        if any(input1 == input2(i))
            numchar = numchar+1;
        end
    end
end