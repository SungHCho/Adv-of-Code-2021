...
clc;
clear;
FID = fopen('Day4Data.txt');
CalledNum = str2double(split(fgetl(FID),','));

Boards = zeros(5);
BoardCount = 0;
LineCount = 1;

% READ DATA, GOD DAMN THIS SUCKS
while ~feof(FID)
    LineData = strrep(strtrim(fgetl(FID)),'  ', ' ');
    
    if ~isempty(LineData)
        Boards(LineCount, :, BoardCount) = str2double(split(LineData,' '));
...
BingoBool = false(size(Boards));
Bingo = false;
for CurrNum = 1:length(CalledNum)
    BingoBool = or(BingoBool,Boards == CalledNum(CurrNum));
    
    for CurrBoard = 1:BoardCount
        for CurrLine = 1:size(Boards,1)
            if all(BingoBool(CurrLine,:,CurrBoard)) ||...
                    all(BingoBool(:,CurrLine,CurrBoard))
                Bingo = true;
                break;
            end
        end
...
WinBoard = Boards(:,:,CurrBoard);
Score = sum(WinBoard(~BingoBool(:,:,CurrBoard)))*CalledNum(CurrNum);

fprintf('Winning board''s score is: %i\n',Score);
...
% a sum of unmarked numbers equal to 148 for a final score of 148 * 13 =
% 1924.
% 
% Figure out which board will win last. Once it wins, what would its final
% score be?
Bingo = false(1,BoardCount);
BingoBool = false(size(Boards));
for CurrNum = 1:length(CalledNum)
    BingoBool = or(BingoBool,Boards == CalledNum(CurrNum));
...
LastBoard = Boards(:,:,CurrBoard);
LastScore = sum(LastBoard(~BingoBool(:,:,CurrBoard)))*CalledNum(CurrNum);

fprintf('Last winning board''s score is: %i\n',LastScore);