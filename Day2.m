...
% the depth by X units. up X decreases the depth by X units. Note that
% since you're on a submarine, down and up affect your depth, and so they
...
clc;
clear;
DataIn = splitlines(fileread('Day2Data.txt'));

% Part 1
SplitData = cell(length(DataIn),2); % Preallocate memory
pos = 0;
depth = 0;
for i = 1:length(DataIn)
    SplitData(i,:) = strsplit(DataIn{i},' '); % Split at space 
    SplitData{i,2} = str2double(SplitData{i,2}); % string to number
    switch SplitData{i,1}
        case 'forward'
            pos = pos + SplitData{i,2};
            
        case 'down'
            depth = depth + SplitData{i,2};
...
aim = 0;
for i = 1:length(DataIn)
    switch SplitData{i,1} % Use processed data from part 1
        case 'forward'
            pos = pos + SplitData{i,2};
            depth = depth + aim*SplitData{i,2};
            
        case 'down'
            aim = aim + SplitData{i,2};
...
fprintf('Final Pos*Depth in part 2 is %i.\n', pos*depth)
