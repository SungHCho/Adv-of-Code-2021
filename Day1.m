...
%   ocean! Before you know it, you're inside a submarine the Elves keep
%   ready for situations like this. It's covered in Christmas lights
...
%   strength by displaying 0-50 stars. Your instincts tell you that in
%   order to save Christmas, you'll need to get all fifty stars by December
%   25th. Collect stars by solving puzzles. Two puzzles will be made
%   available on each day in the Advent calendar; the second puzzle is
%   unlocked when you complete the first. Each puzzle grants one star. Good
%   luck! As the submarine drops below the surface of the ocean, it
...
%   line is a measurement of the sea floor depth as the sweep looks further
...
%   sonar sweep found depths of 199, 200, 208, 210, and so on. The first
%   order of business is to figure out how quickly the depth increases,
%   just so you know what you're dealing with - you never know if the keys
%   will get carried into deeper water by an ocean current or a fish or
%   something. To do this, count the number of times a depth measurement
%   increases from the previous measurement. (There is no measurement
%   before the first measurement.) In the example above, the changes are as
%   follows:
%   ```
...
%   previous measurement. How many measurements are larger than the
%   previous measurement?

clc;
clear;
FID = fopen('Day1Data.txt');
DataIn = fscanf(FID,'%i');
fclose(FID);

% Part 1
IncreaseCount = sum(diff(DataIn)>0);
fprintf('Data increases %i times.\n', IncreaseCount)

% --- Part Two ---
% Considering every single measurement isn't as useful as
...
% Part 2
MovSum = movsum(DataIn,3);
MovSumIncCount = sum(diff(MovSum(2:end-1))>0);
fprintf('Data increases %i times in 3 point moving sum.\n', MovSumIncCount)